...
export default function SeoFormPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (isEdit && recordId) {
        res = await axios.put(
          `${process.env.NEXT_PUBLIC_BASE_URL}/api/seo/${recordId}`,
          payload,
          {
            headers: {
              Authorization: `Bearer ${Cookies.get("genx_access_token")}`,
            },
          }
        );
      } else {
              ...
              <div className="block lg:hidden w-full mt-4 ml-2">
                <div className="flex items-center gap-2">
                  <HiArrowLeft
                    ...
                    onClick={() => router.back()}
                  />
                  <h1 className="text-xl font-bold "> SEO Configuration</h1>
                </div>
              </div>
