...
const CvCreators = () => {
  ...
  const downloadusersExcel = async () => {
    try {
      const token = Cookies.get("genx_access_token");
      const response = await axios.get(
        `${process.env.NEXT_PUBLIC_BASE_URL}/api/cv-creation-list/export`,
        {
          responseType: "blob",
          headers: {
            Authorization: `Bearer ${token}`,
      ...
      toast.error(
        error.response.data.message ||
          "Failed to download Cv Creators Excel file. Please try again later."
      );
    }
  };
              ...
              <div className="block lg:hidden w-full mt-4 ml-2">
                <div className="flex items-center gap-2">
                  <HiArrowLeft
                    ...
                    onClick={() => router.back()}
                  />
                  <h1 className="text-xl font-bold "> CV Creators</h1>
                </div>
              </div>
